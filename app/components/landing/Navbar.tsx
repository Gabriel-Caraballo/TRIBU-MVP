...
export const Navbar: React.FC<NavbarProps> = () => {
  ...
  return (
    <>
      ...
      {/* Menú móvil */}
      <div 
        className={`fixed inset-0 z-40 flex flex-col items-center justify-center bg-[#0a0a09] space-y-8 p-4 pt-20 transition-transform duration-300 ease-in-out transform md:hidden overflow-y-auto ${
      ...
      >
        ...
        <div className="flex flex-col space-y-4 w-full max-w-xs mt-8">
          <Link
            href="/auth/login"
            className="py-4 text-[#e8e8e2] border border-white/10 text-center w-full font-bold uppercase tracking-widest text-xs rounded-none"
            onClick={() => setIsMenuOpen(false)}
          >
            Iniciar sesión
          </Link>
          <Link
            href="/auth/register"
            className="py-4 bg-[--tribu-green] text-white text-center w-full font-bold uppercase tracking-widest text-xs rounded-none"
            onClick={() => setIsMenuOpen(false)}
          >
            Registrarme
          </Link>
